...
class Cohesityrecoveries(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        if number > 0:
            _log.info('There are ' + str(number) + ' recoveries in the last 30 days')
        else:
            _log.info('No recoveries in the last 30 days')

        metric = nagiosplugin.Metric('Alerts with issues', number, min=0, context='warning')
        return metric
...
def parse_args():
    ...
    argp.add_argument('-w', '--warning', metavar='RANGE', default='~:0', help='return warning if occupancy is outside RANGE. Value is expressed in number of recoveries')
...
def main():
    ...
    args = parse_args()
    check = nagiosplugin.Check( Cohesityrecoveries(args.ip, args.user, args.password) )
