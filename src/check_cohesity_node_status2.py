...
class Cohesitynodestatus(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        if countt_node[0] == countt_node[1]:
            _log.info('All ' + str(countt_node[0]) + ' nodes are active on the cluster')
        else:
            _log.debug(str(difference) +' nodes are not active on cluster')

        metric = nagiosplugin.Metric('Unhealthy nodes', difference, min=0, context='difference')
        return metric
...
def main():
    ...
    check.add(nagiosplugin.ScalarContext('difference', args.warning, args.critical))
