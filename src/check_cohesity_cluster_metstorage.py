...
class Cohesityclusterstorage(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        _log.info('Cluster Metadata storage is '+str(percent_used)+'% used')
        metric = nagiosplugin.Metric('Cluster used Metadata storage', percent_used, '%', min=0, max=100, context='cluster_used')
        return metric
